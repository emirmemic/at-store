import { IconProps } from '@/lib/types';

export default function IconFacebook({
  className = '',
  pathClassName,
  ...props
}: IconProps) {
  return (
    <svg
      className={className}
      fill="currentColor"
      height="25"
      viewBox="0 0 25 25"
      width="25"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path
        className={pathClassName ?? 'text-blue'}
        d="M25 12.5C25 5.59648 19.4035 0 12.5 0C5.59648 0 0 5.59648 0 12.5C0 18.7391 4.57109 23.9104 10.5469 24.8481V16.1133H7.37305V12.5H10.5469V9.74609C10.5469 6.61328 12.4131 4.88281 15.2684 4.88281C16.6359 4.88281 18.0664 5.12695 18.0664 5.12695V8.20312H16.4902C14.9374 8.20312 14.4531 9.1667 14.4531 10.1553V12.5H17.9199L17.3657 16.1133H14.4531V24.8481C20.4289 23.9104 25 18.7392 25 12.5Z"
        fill={'currentColor'}
      />
      <path
        d="M17.3677 16.1133L17.9219 12.5H14.4551V10.1553C14.4551 9.1666 14.9394 8.20312 16.4922 8.20312H18.0684V5.12695C18.0684 5.12695 16.6379 4.88281 15.2702 4.88281C12.415 4.88281 10.5488 6.61328 10.5488 9.74609V12.5H7.375V16.1133H10.5488V24.8481C11.1949 24.9494 11.848 25.0002 12.502 25C13.1559 25.0002 13.809 24.9494 14.4551 24.8481V16.1133H17.3677Z"
        fill="currentColor"
      />
    </svg>
  );
}
