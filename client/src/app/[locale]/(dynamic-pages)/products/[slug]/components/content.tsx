...
export default function Content(productData: ProductResponse) {
  const { details, name, originalPrice, discountedPrice } = productData;
  const finalPrice = discountedPrice ?? originalPrice;
  const image = productData.images?.[0] || null;
  return (
    <main className="flex flex-col gap-8 py-8 container-max-width md:py-20">
